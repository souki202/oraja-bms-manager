...
import { parseDroppedChartVariants } from './chartMetadata';
...
export class ManagerRepository {
  ...
  async analyzeDroppedChart(paths: string[]): Promise<ChartImportAnalysis> {
    ...
    const variants = parseDroppedChartVariants(supportedPath, buffer);
    const analyses = variants
    ...
    if (!best || best.candidates.length === 0) {
      return {
        ...
        dropped: best?.dropped ?? variants[0] ?? null,
        candidates: [],
        sourcePaths: existingPaths,
        companionPaths
      };
